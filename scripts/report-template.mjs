...
export function buildReport(S, {assets}) {
  ...
  /* Сценарии, которым нужен тариф выше. ДВЕ причины, один порог:
       10, 11 — держатся на AI-агентах (agent:true в index.html). Было пять; с
                ...
                функции доступными ниже Professional (открытые вопросы, §20);
       5 — переведён на порог Professional по строгому чтению противоречивой
           строки официальной страницы (minPro:true, открытые вопросы, §19).
...
/* ---------- assemble ----------
   ЛИСТЫ ЖИВУТ МАССИВОМ, И ЭТОТ ЖЕ МАССИВ ИХ СЧИТАЕТ. Раньше листы склеивались
   сразу, а число листов добывалось регуляркой /<section class="page/ по ГОТОВОМУ
   файлу — вместе с кодом отчёта в нём лежит вклеенный кит, а в его комментарии
   стоит строка-пример `<section class="page page--li page--partner-navy">`.
   Она попадала в счёт, и build-report печатал на лист больше, чем в документе:
   постоянное завышение на единицу, безобидное на вид. Безобидным оно не было —
   то же число уехало в инструкцию партнёру («between fourteen and seventeen
   sheets» при настоящих 13 и 16), потому что проверка инструкции считала листы
   ТОЙ ЖЕ регуляркой и подтверждала завышенное значение завышенным.
   Считать надо то, что собрано, а не совпадения в тексте файла. Каждая функция
   отдаёт массив листов, длина массива и есть число листов. */
const sheets = [
  cover(),
  headline(),
  scenarioPages(),
